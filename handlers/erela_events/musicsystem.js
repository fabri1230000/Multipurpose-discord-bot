...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!member.voice.channel) return interaction.reply({
      ephemeral: true,
      content: "<a:no14:949132530064433173> **Primero conéctese a un canal de voz!**"
    })
    //now its time to start the music system
    if (!member.voice.channel)
      return interaction.reply({
        content: `<a:no14:949132530064433173>  **Únase primero a un canal de voz!**`,
        ephemeral: true
      })
    ...
    if (player && member.voice.channel.id !== player.voiceChannel)
      return interaction.reply({
        content: `<a:no14:949132530064433173> **Únase primero al __mi__ canal de voz! <#${player.voiceChannel}>**`,
        ephemeral: true
      })
    if (interaction.isButton()) {
      if (!player || !player.queue || !player.queue.current) {
        return interaction.reply({
          content: "<a:no14:949132530064433173> Nada Jugando todavía",
          ephemeral: true
        })
      }
      ...
      if (player && interaction.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
        return interaction.reply({
          embeds: [new MessageEmbed()
            ...
            .setTitle(`<a:no14:949132530064433173> **No eres un DJ ni el Song Requester!**`)
        ...
        case "Skip": {
          ...
          if (!player.queue || !player.queue.size || player.queue.size === 0) {
            ...
            interaction.reply({
              embeds: [new MessageEmbed()
                ...
                .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({
                  dynamic: true
                })))
              ]
            })
          ...
          interaction.reply({
            embeds: [new MessageEmbed()
              .setColor(es.color)
              .setTimestamp()
              .setTitle(`⏭ **Saltó a la siguiente canción!**`)
              .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({
                dynamic: true
              })))
            ]
      ...
      case "Autoplay": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.get(`autoplay`) ? `<a:ys2:934869188349161492> **Reproducción automática habilitada**`: `<a:no14:949132530064433173> **Reproducción automática deshabilitada**`}`)
            .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Song": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.trackRepeat ? `<a:ys2:934869188349161492> **Bucle de canción habilitado**`: `<a:no14:949132530064433173> **Bucle de canción deshabilitado**`}`)
      ...
      case "Queue": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.queueRepeat ? `<a:ys2:934869188349161492> **Bucle de cola habilitado**`: `<a:no14:949132530064433173> **Bucle de cola deshabilitado**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Rewind": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`⏪ **Rebobinado la canción para \`10 Segundos\`!**`)
            .setFooter(client.getFooter(`💢 Acción por: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
    ...
    if (interaction.isSelectMenu()) {
      ...
      interaction.reply({
        embeds: [new MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor(`Cargando '${interaction.values[0] ? interaction.values[0] : "Default"}' Mezcla de música`, "https://imgur.com/xutrSuq.gif", link))
  ...
  client.on("messageCreate", async message => {
    ...
    if (!channel) return message.reply("<a:no14:949132530064433173> **Únase primero a un canal de voz!**").then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
    ...
    if (player && channel.id !== player.voiceChannel) return message.reply(`<a:no14:949132530064433173> **Únase primero al __mi__ canal de voz! <#${player.voiceChannel}>**`).then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  var embeds = [
    ...
    .setDescription(`**No hay canciones en cola**`)
    ...
    .setTitle(`Únase a un canal de voz para reproducir música`)
    .setDescription(`> Apoyo <:canal:919011403954814997> Youtube, <:I_spotify:965052296964767804> Spotify y <:nota:965052349242564648> ¡Muchos más!
    ¡Acabo de enviar el nombre o el enlace de la canción a este canal para reproducir la música`)
  ]
  ...
  var stopbutton = new MessageButton().setStyle('DANGER').setCustomId('Stop').setEmoji(`938854198504738837`).setLabel(`Stop`).setDisabled()
  var skipbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Skip').setEmoji(`938854193916170281`).setLabel(`Skip`).setDisabled();
  var shufflebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Shuffle').setEmoji('🔀').setLabel(`Shuffle`).setDisabled();
  var pausebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Pause').setEmoji('938854180272103454').setLabel(`Pause`).setDisabled();
  var autoplaybutton = new MessageButton().setStyle('SECONDARY').setCustomId('Autoplay').setEmoji('938855831678648410').setLabel(`Autoplay`).setDisabled();
  var songbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Song').setEmoji("938855831678648410").setLabel(`Song`).setDisabled();
  var queuebutton = new MessageButton().setStyle('SECONDARY').setCustomId('Queue').setEmoji(`🔂`).setLabel(`Queue`).setDisabled();
  var forwardbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Forward').setEmoji('938855831678648410').setLabel(`+10 Sec`).setDisabled();
  var rewindbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Rewind').setEmoji('938854193916170281').setLabel(`-10 Sec`).setDisabled();
